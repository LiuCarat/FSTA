...
def train(data_loader, ground_truth, device, opt):
    ...
    for epoch_i in range(opt.epoch):
        ...
        if epoch_i % 100 == 0:
            print(f'index:{opt.index}, epoch:{epoch_i}, loss:{train_loss: .3f}, precision:{precision}, recall:{recall}, F1:{F1}, accuracy:{accuracy}, SHD:{SHD}')
