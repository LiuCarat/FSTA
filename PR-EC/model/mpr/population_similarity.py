...
def load_aligned_phenotypes(csv_path, subject_ids, columns, profile=None):
    ...
    with Path(csv_path).open(newline="", encoding="utf-8-sig") as handle:
        ...
        rows = {
            ...
            for row in reader
            if row.get(identifier, "").strip() and row[identifier].strip() != "no_filename"
        }
...
def fused_graph(
    ...
    population_similarity = np.asarray(population_similarity, dtype=np.float32)
    if fmri_graph.shape != population_similarity.shape:
        raise ValueError(
            f"Graph shapes must match, got {fmri_graph.shape} and {population_similarity.shape}"
        )
    scores = float(beta) * fmri_graph + (1.0 - float(beta)) * population_similarity
