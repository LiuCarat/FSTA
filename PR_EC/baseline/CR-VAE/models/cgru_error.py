...
class CRVAE(nn.Module):
    ...
    def forward(self, X, noise = None, mode = 'train', phase = 0):
        ...
        if phase == 0:
            X = torch.cat((torch.zeros(X.shape, device=device)[:,0:1,:],X),1)
            if mode == 'train':
                
    
                hidden_0 = torch.zeros(
                    1, X.shape[0], self.hidden, device=device
                )
                _, h_t = self.gru_left(X[:,1:11,:], hidden_0.detach())
                ...
                return pred, log_var, mu
            if mode == 'test':
                X_seq = torch.zeros(X[:,:1,:].shape, device=device)
                h_0 = torch.randn(size = (1, X_seq[:,-2:-1,:].size(0),self.hidden), device=device)
                ht_last =[]
                for i in range(self.p):
                    ht_last.append(h_0)
