...
from downstream_adhd200.data import ROI_COUNT, load_adhd_records, load_time_series
...
def parse_args():
    ...
    parser.add_argument(
        "--data_root",
        default=str(REPO_ROOT / "dataset/ADHD200"),
    )
    parser.add_argument(
        "--output_root",
        default=str(REPO_ROOT / "downstream_adhd200/outputs/FSTA_BEC"),
    )
    parser.add_argument("--file_prefix", default="sfnwmrda")
    ...
    parser.add_argument("--window_length", type=int, default=72)
...
def main():
    ...
    args = parse_args()
    if args.window_length != 72:
        raise ValueError("The ADHD200 FSTA_BEC version is fixed to window_length=72")
    ...
    # 加载 ADHD200 受试者记录和时间序列数据
    records = load_adhd_records(
