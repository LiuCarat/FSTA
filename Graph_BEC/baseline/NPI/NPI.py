...
def model_EC(model, input_X, target_Y, pert_strength):
    ...
    The returned matrix uses ``[source, target]`` axes: row ``source`` is
    the node perturbed in the latest input time point, and column ``target``
    is the corresponding change in the model output.
    """
    ...
    for source_node in range(node_num):
        ...
        perturbation = np.zeros((steps, node_num)); perturbation[-1, source_node] = pert_strength
        perturbed_output = model(torch.tensor(input_X + perturbation.flatten(), dtype = torch.float).to(device)).detach().cpu().numpy()
        NPI_EC[source_node] = np.mean(perturbed_output - unperturbed_output, axis = 0)
    return NPI_EC
