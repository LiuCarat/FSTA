"""Generate subject-level Pearson FC matrices and run the BEC classifier probe.

This baseline keeps the downstream evaluation identical to Graph-BEC. Only the
subject-level representation changes: FSTA/Graph-BEC BEC is replaced by the
Pearson correlation matrix of each subject's ROI time series.
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--fc-path",
        type=Path,
        default=output_dir / f"subject_fc_{profile.name}.npz",
    )
...
def generate_pearson_fc(series):
    """Return a finite, zero-diagonal Pearson correlation matrix.

    A zero-variance ROI has undefined Pearson correlations. Such an ROI is
    represented by an all-zero row and column instead of propagating NaNs.
    """
    if series.ndim != 2:
    ...
    if np.count_nonzero(variable) > 0:
        variable_fc = np.atleast_2d(
            np.corrcoef(series[:, variable], rowvar=False)
        ).astype(np.float32, copy=False)
        matrix[np.ix_(variable, variable)] = variable_fc
    if matrix.ndim != 2 or matrix.shape[0] != matrix.shape[1]:
        raise ValueError(f"Expected a square FC matrix, got {matrix.shape}")
    if not np.isfinite(matrix).all():
        raise ValueError("Pearson FC contains non-finite values")
...
def save_fc_archive(path, fc, dataset, args):
    ...
    np.savez_compressed(
        ...
        representation=np.asarray("pearson_fc"),
...
def load_fc_archive(path):
    with np.load(path, allow_pickle=False) as archive:
        ...
        if missing:
            raise ValueError(f"Missing Pearson FC arrays: {sorted(missing)}")
...
def generate_fc_archive(args, dataset):
    if args.fc_path.is_file() and not args.regenerate_fc:
        ...
        if len(archive["bec"]) == len(dataset["time_series"]):
            print(f"using existing Pearson FC archive: {args.fc_path}")
            return archive
        raise ValueError("Existing Pearson FC archive is incomplete; use --regenerate-fc")

    matrices = []
    total = len(dataset["time_series"])
    for index, series in enumerate(dataset["time_series"], start=1):
        matrix = generate_pearson_fc(series)
        matrices.append(matrix)
        if index % 100 == 0 or index == total:
            print(f"Pearson FC [{index}/{total}]")
...
def main():
    ...
    print(f"Pearson FC archive: {args.fc_path} shape={archive['bec'].shape}")
