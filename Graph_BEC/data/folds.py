...
def _encode_category_splits(train_cat, val_cat, test_cat):
    ...
    for column in range(train_cat.shape[1]):
        ...
        encode = lambda values: np.asarray([categories.get(value, unknown) for value in values.astype(str)])
...
def prepare_fold_arrays(train_bec, val_bec, test_bec, train_cont, val_cont,
    ...
    encoded_train, encoded_val, encoded_test = _encode_category_splits(train_cat, val_cat, test_cat)
    return {
